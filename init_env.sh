...
{ 
  ...
  if [ -n "${SEGMENT_ACCESS_KEYFILE}" ]; then
    ./multiscp.sh -k ${SEGMENT_ACCESS_KEYFILE} -f $HOSTS_FILE -u $USER init_env_segment.sh /tmp
    ./multiscp.sh -k ${SEGMENT_ACCESS_KEYFILE} -f $HOSTS_FILE -u $USER deploycluster_parameter.sh /tmp
    ./multiscp.sh -k ${SEGMENT_ACCESS_KEYFILE} -f $HOSTS_FILE -u $USER /tmp/hostsfile /tmp
    ./multiscp.sh -k ${SEGMENT_ACCESS_KEYFILE} -f $HOSTS_FILE -u $USER /home/${ADMIN_USER}/.ssh/id_rsa.pub /tmp
    ./multiscp.sh -k ${SEGMENT_ACCESS_KEYFILE} -f $HOSTS_FILE -u $USER ${CLOUDBERRY_RPM} ${CLOUDBERRY_RPM}
  else
    ./multiscp.sh -p ${SEGMENT_ACCESS_PASSWORD} -f $HOSTS_FILE -u $USER init_env_segment.sh /tmp
    ./multiscp.sh -p ${SEGMENT_ACCESS_PASSWORD} -f $HOSTS_FILE -u $USER deploycluster_parameter.sh /tmp
    ./multiscp.sh -p ${SEGMENT_ACCESS_PASSWORD} -f $HOSTS_FILE -u $USER /tmp/hostsfile /tmp
    ./multiscp.sh -p ${SEGMENT_ACCESS_PASSWORD} -f $HOSTS_FILE -u $USER /home/${ADMIN_USER}/.ssh/id_rsa.pub /tmp
    ./multiscp.sh -p ${SEGMENT_ACCESS_PASSWORD} -f $HOSTS_FILE -u $USER ${CLOUDBERRY_RPM} ${CLOUDBERRY_RPM}
  fi
...
function init_segment()
{
  ...
  if [ -n "${SEGMENT_ACCESS_KEYFILE}" ]; then
    ./multissh.sh -k ${SEGMENT_ACCESS_KEYFILE} -f $HOSTS_FILE -u $USER "$COMMAND"
  else
    ./multissh.sh -p ${SEGMENT_ACCESS_PASSWORD} -f $HOSTS_FILE -u $USER "$COMMAND"
  fi
