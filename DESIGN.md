...
│  ┌─ 高级选项 (点击展开) ──────────────────────────── ▶ ───┐  │
│  │  数据库端口       [5432  ]                             │  │
│  │  Coordinator 目录 [/data0/coordinator/gpseg-1     ]    │  │
│  │  数据目录         [/data0/primary                 ]    │  │
│  │  Segment 数量     [2     ] (每主机)                    │  │
│  │  Segment 起始端口 [6000  ]                             │  │
│  │  集群名称         [gp    ]                             │  │
...
**交互逻辑:**
- 多节点时显示 Segment 主机列表，支持动态增删行
- 主机名为空时，可根据 IP 自动生成 (sdw1, sdw2, ...)
- 高级选项默认折叠，展开后显示全部参数，均有合理默认值
- 点击"下一步"时校验所有字段，调用 `/save_config` 保存
